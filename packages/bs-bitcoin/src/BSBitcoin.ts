...
export class BSBitcoin implements IBSBitcoin {
  ...
  async #getTransferData({ senderAccount, intents, shouldValidate = true }: TGetTransferDataParams) {
    ...
    if (!feeBn) {
      throw new BSError("Fee can't be calculated", 'FEE_CALCULATION_FAILED')
    }
      ...
      return [
        {
          ...
          inputs: selectedUtxos.map(utxo => ({
            address,
            addressUrl,
            amount: new BSBigHumanAmount(utxo.value, BSBitcoinConstants.NATIVE_TOKEN.decimals).toFormatted(),
