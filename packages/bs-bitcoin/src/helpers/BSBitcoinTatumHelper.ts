import { BSError, type TBSNetwork } from '@cityofzion/blockchain-service'
import type { TBSBitcoinNetworkId, TTatumApis } from '../types'
import { BSBitcoinHelper } from './BSBitcoinHelper'
...
export class BSBitcoinTatumHelper {
  ...
  static #apis?: TTatumApis
  static #currentNetwork?: TBSNetwork<TBSBitcoinNetworkId>

  static getApis(network: TBSNetwork<TBSBitcoinNetworkId>): TTatumApis {
    const isMainnet = BSBitcoinHelper.isMainnetNetwork(network)

    if (!isMainnet && !BSBitcoinHelper.isTestnetNetwork(network)) {
      throw new BSError('Only mainnet and testnet are supported', 'INVALID_NETWORK')
    }

    if (!this.#apis || this.#currentNetwork?.type !== network.type) {
