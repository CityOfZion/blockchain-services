import {
  ...
  type ILedgerService,
  type TBSAccount,
  type TBSUtilsHelperRetryOptions,
...
export class LedgerServiceBitcoin<N extends string = string> implements ILedgerService<N> {
  readonly #retryOptions: TBSUtilsHelperRetryOptions = {
    retries: 10,
    delay: 500,
  }
  ...
  async getAccount(transport: Transport, index: number): Promise<TBSAccount<N>> {
    ...
    const address = await BSUtilsHelper.retry(
      async () => {
        const { bitcoinAddress } = await ledgerApp.getWalletPublicKey(bipPathWithoutMasterKey, { format: 'bech32' })
  ...
  async signTransaction(psbt: bitcoinjs.Psbt, account: TBSAccount<N>, transport: Transport): Promise<void> {
    try {
      ...
      const signedTransactionHex = await BSUtilsHelper.retry(
        async () =>
          await ledgerApp.createPaymentTransaction({
            inputs,
          ...
          }),
        {
          ...
          shouldRetry: (error: any) => {
            ...
            return (
              ...
              (name === 'TransportStatusError' && ['UNKNOWN_ERROR', 'UNKNOWN_APDU'].includes(error?.statusText))
            )
          },
        }
      )
