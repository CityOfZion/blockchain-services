...
describe('BlockscoutBDSEthereumFullTransactionsByAddress', () => {
  describe('getFullTransactionsByAddress', () => {
    const mainnetNetwork = BSEthereumConstants.NEOX_MAINNET_NETWORK
    const testnetNetwork = BSEthereumConstants.NEOX_TESTNET_NETWORK
    ...
    let blockscoutBDSEthereum: BlockscoutBDSEthereum
    ...
    beforeEach(() => {
      ...
      nftDataService = new GhostMarketNDSEthereum(mainnetNetwork) as jest.Mocked<GhostMarketNDSEthereum>

      nftDataService.getNft = jest
        .fn()
        .mockReturnValue({ image: 'nftImage', name: 'nftName', collectionName: 'nftCollectionName' })

      explorerService = new BlockscoutESEthereum(mainnetNetwork) as jest.Mocked<BlockscoutESEthereum>
      ...
      blockscoutBDSEthereum = new BlockscoutBDSEthereum(mainnetNetwork, nftDataService, explorerService)
    })
    ...
    it("Shouldn't be able to get transactions when is using a different network (Custom) from Neo X Mainnet and Neo X Testnet", async () => {
      nftDataService = new GhostMarketNDSEthereum(otherNetwork)
      blockscoutBDSEthereum = new BlockscoutBDSEthereum(otherNetwork, nftDataService, explorerService)

      await expect(blockscoutBDSEthereum.getFullTransactionsByAddress(params)).rejects.toThrow(
        'This network is not supported'
      )
    })

    it("Shouldn't be able to get transactions when missing one of the dates", async () => {
      await expect(blockscoutBDSEthereum.getFullTransactionsByAddress({ ...params, dateFrom: '' })).rejects.toThrow(
        'Missing dateFrom param'
      )

      await expect(blockscoutBDSEthereum.getFullTransactionsByAddress({ ...params, dateTo: '' })).rejects.toThrow(
    ...
    it("Shouldn't be able to get transactions when one of the dates is invalid", async () => {
      await expect(
        blockscoutBDSEthereum.getFullTransactionsByAddress({ ...params, dateFrom: 'invalid' })
      ).rejects.toThrow('Invalid dateFrom param')

      await expect(
        blockscoutBDSEthereum.getFullTransactionsByAddress({ ...params, dateTo: 'invalid' })
    ...
    it("Shouldn't be able to get transactions when dateFrom is greater than dateTo", async () => {
      ...
      await expect(
        blockscoutBDSEthereum.getFullTransactionsByAddress({
          ...params,
    ...
    it("Shouldn't be able to get full transactions when address is wrong", async () => {
      await expect(
        blockscoutBDSEthereum.getFullTransactionsByAddress({ ...params, address: 'invalid' })
    ...
    it("Shouldn't be able to get transactions when the range dates are greater than one year", async () => {
      ...
      await expect(
        blockscoutBDSEthereum.getFullTransactionsByAddress({ ...params, dateFrom: dateFrom.toJSON() })
    ...
    it('Should be able to get transactions when is using a Neo X Testnet network', async () => {
      nftDataService = new GhostMarketNDSEthereum(testnetNetwork)
      blockscoutBDSEthereum = new BlockscoutBDSEthereum(testnetNetwork, nftDataService, explorerService)

      const response = await blockscoutBDSEthereum.getFullTransactionsByAddress({
        ...params,
    ...
    it.skip('Should be able to get transactions when send the nextCursor param using Neo X Mainnet network', async () => {
      ...
      const response = await blockscoutBDSEthereum.getFullTransactionsByAddress(newParams)
      const nextResponse = await blockscoutBDSEthereum.getFullTransactionsByAddress({
        ...newParams,
    ...
    // it('Should be able to get transactions with NFTs when it was called using Neo X Mainnet network', async () => {
    //   const response = await blockscoutBDSEthereum.getFullTransactionsByAddress({
    //     ...params,
