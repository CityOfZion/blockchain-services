  ...
  implements IBlockchainDataService<N>
{
  ...
  async getTransaction(hash: string): Promise<TTransaction<N>> {
    ...
    const fromUrl = transaction.from ? addressTemplateUrl?.replace('{address}', transaction.from) : undefined
    const toUrl = transaction.to ? addressTemplateUrl?.replace('{address}', transaction.to) : undefined
