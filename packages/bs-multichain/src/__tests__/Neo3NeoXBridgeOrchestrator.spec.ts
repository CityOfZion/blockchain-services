...
describe('Neo3NeoXBridgeOrchestrator', () => {
  ...
  it('Should be able to set address to receive to a invalid address', async () => {
    ...
    const setAmountToReceiveSpy = jest.spyOn(neo3NeoXBridgeOrchestrator.toService, 'validateAddress')
    ...
    expect(setAmountToReceiveSpy).toHaveBeenCalledTimes(1)
    expect(setAmountToReceiveSpy).toHaveBeenCalledWith(receiverAddress)
  })
  ...
  it('Should be able to set address to receive to a valid address', async () => {
    ...
    expect(setAmountToReceiveSpy).toHaveBeenCalledTimes(1)
    expect(setAmountToReceiveSpy).toHaveBeenCalledWith(toAccount.address)
  })
  ...
  it.skip('Should not be able to wait if it was not bridge', async () => {
    await neo3NeoXBridgeOrchestrator.init()
