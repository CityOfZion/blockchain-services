...
describe('DoraBDSNeo3Vote', () => {
  ...
  describe('getCandidatesToVote', () => {
    it("Shouldn't be able to get candidates to vote when is using a Testnet network", async () => {
      bsNeo3 = new BSNeo3('neo3', testnetNetwork)

      await expect(bsNeo3.getCandidatesToVote()).rejects.toThrow('Only Mainnet is supported')
    })

    it('Should be able to get candidates to vote when is using a Mainnet network', async () => {
      const response = await bsNeo3.getCandidatesToVote()
  ...
  describe('getVoteDetailsByAddress', () => {
    ...
    it("Shouldn't be able to get vote details by address when is using a Testnet network", async () => {
      ...
      await expect(bsNeo3.getVoteDetailsByAddress(address)).rejects.toThrow('Only Mainnet is supported')
    })

    it("Shouldn't be able to get vote details by address when there isn't address", async () => {
      await expect(bsNeo3.getVoteDetailsByAddress('')).rejects.toThrow('Missing address')
    })

    it("Shouldn't be able to get vote details by address when address is invalid", async () => {
      await expect(bsNeo3.getVoteDetailsByAddress('invalidAddress')).rejects.toThrow('Invalid address')
    })

    it('Should be able to get vote details by address when address is valid', async () => {
      const response = await bsNeo3.getVoteDetailsByAddress(address)
  ...
  describe('calculateVoteFee', () => {
    it("Shouldn't be able to calculate vote fee when is using a Testnet network", async () => {
      bsNeo3 = new BSNeo3('neo3', testnetNetwork)

      await expect(bsNeo3.calculateVoteFee({ account, candidatePubKey: cozCandidatePubKey })).rejects.toThrow(
        'Only Mainnet is supported'
      )
    })

    it("Shouldn't be able to calculate vote fee when there isn't candidatePubKey", async () => {
      await expect(bsNeo3.calculateVoteFee({ account, candidatePubKey: '' })).rejects.toThrow(
        'Missing candidatePubKey param'
      )
    })

    it('Should be able to calculate vote fee', async () => {
      const fee = await bsNeo3.calculateVoteFee({ account, candidatePubKey: cozCandidatePubKey })
  ...
  describe('vote', () => {
    it("Shouldn't be able to vote when is using a Testnet network", async () => {
      ...
      await expect(bsNeo3.vote({ account, candidatePubKey: cozCandidatePubKey })).rejects.toThrow(
        'Only Mainnet is supported'
      )
    })

    it("Shouldn't be able to vote when there isn't candidatePubKey", async () => {
      await expect(bsNeo3.vote({ account, candidatePubKey: '' })).rejects.toThrow('Missing candidatePubKey param')
    })

    it.skip('Should be able to vote with success', async () => {
      const { transactionHash } = await bsNeo3.vote({ account, candidatePubKey: cozCandidatePubKey })
    ...
    it.skip('Should be able to vote using Ledger with success', async () => {
      ...
      bsNeo3 = new BSNeo3('neo3', undefined, async () => transport)
