    ...
    BSWithExplorerService
{
  ...
  async #buildTransferParams(param: TransferParam) {
    ...
    for (const intent of param.intents) {
      ...
      const normalizedTokenHash = normalizeHash(intent.tokenHash, false)

      const isNative = normalizedTokenHash === normalizeHash(BSSolanaConstants.NATIVE_TOKEN.hash, false)
      if (isNative) {
        transaction.add(
