...
describe('NeonDappKitLedgerServiceNeo3', () => {
  beforeAll(async () => {
    ...
    accounts.forEach((account, index) => {
      expect(account).toEqual(
        expect.objectContaining({
          ...
          type: 'publicKey',
          bip44Path: bsSolana.bip44DerivationPath.replace('?', index.toString()).replace('m/', ''),
        })
      )
    })
  }, 60000)
  ...
  it('Should be able to get account', async () => {
    ...
    expect(account).toEqual(
      expect.objectContaining({
        ...
        key: expect.any(String),
        type: 'publicKey',
        bip44Path: bsSolana.bip44DerivationPath.replace('?', '0').replace('m/', ''),
      })
    )
  }, 60000)
