...
export class TatumRpcBDSSolana implements BlockchainDataService {
  ...
  async #parseSplTransferCheckedInstruction(
    ...
    const amount = formatTokenNumber(info.tokenAmount.amount, token.decimals)
  ...
  async #parseSystemInstruction(
    ...
    const amount = formatTokenNumber(info.lamports, BSSolanaConstants.NATIVE_TOKEN.decimals)
  ...
  async getTransaction(txid: string): Promise<TransactionResponse> {
    ...
    return {
      ...
      transfers,
      fee: formatTokenNumber(transaction.meta.fee, this.#feeToken.decimals),
    }
  }
  ...
  async getBalance(address: string): Promise<BalanceResponse[]> {
    ...
    const balances: BalanceResponse[] = [
      {
        amount: formatTokenNumber(nativeBalance, BSSolanaConstants.NATIVE_TOKEN.decimals),
        token: BSSolanaConstants.NATIVE_TOKEN,
      },
    ]
    ...
    const promises = splBalance.value.map(async item => {
      ...
      const amount = formatTokenNumber(item.account.data.parsed.info.tokenAmount.amount, token.decimals)
      if (+amount <= 0) return
