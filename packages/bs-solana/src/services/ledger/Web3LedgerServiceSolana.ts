...
export class Web3LedgerServiceSolana<N extends string = string> implements ILedgerService<N> {
  ...
  constructor(blockchainService: IBSSolana<N>, getLedgerTransport?: GetLedgerTransport<N>) {
  ...
  async signTransaction(transport: Transport, transaction: solanaSDK.Transaction, account: Account<N>) {
